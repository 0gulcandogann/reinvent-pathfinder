...
import type { AgentResponse, AttendeeProfile, AttendeeSchedule, DemoBootstrap, IntegratedSchedule, MutationExecutionResult, ScheduleMutationPlan } from "../lib/types";
...
export function PathfinderApp() {
  ...
  useEffect(() => {
    ...
    if (DEMO_MODE) api.demoState().then((data) => { setBootstrap(data); setExistingSchedule(data.existing_schedule); }).catch(() => setError("Demo API unavailable. Start FastAPI with PATHFINDER_DEMO_MODE=true."));
  }, []);
  ...
  async function optimize(nextProfile = profile) {
    ...
    try {
      const response = await api.agentMessage(`Build my week around ${nextProfile.interests.join(" ")}.`, conversationId.current, profileForRequest(nextProfile), existingSchedule);
  ...
  async function sendMessage(message: string) {
    ...
    try {
      const response = await api.agentMessage(message, conversationId.current);
      absorb(response);
  ...
  async function confirmMutation() {
    if (!DEMO_MODE || !pendingPlan || !pendingPlanId || busy) return;
  ...
  async function dismissMutation() {
    ...
    setBusy(true);
    try {
      const response = await api.agentMessage("Discard plan.", conversationId.current);
  ...
  return <div className="app-shell">
    <header className="app-header"><div className="app-header-inner"><button type="button" onClick={() => setTab("plan")} className="flex shrink-0 items-center gap-2.5 text-left" aria-label="Pathfinder home"><Image src={brandMark} alt="" width={32} height={32} unoptimized priority className="brand-mark" /><span><span className="block text-[15px] font-bold leading-tight tracking-tight">Pathfinder</span><span className="block text-[9px] font-semibold tracking-[.12em] text-muted">RE:INVENT 2026</span></span></button>
      <nav className="app-nav flex items-center gap-1" aria-label="Primary navigation">{nav.map((item) => <button type="button" key={item.id} className={`nav-link ${tab === item.id ? "active" : ""}`} aria-current={tab === item.id ? "page" : undefined} onClick={() => setTab(item.id)}><Icon name={item.icon} size={16} /> {item.label}</button>)}</nav>
      <div className="ml-auto flex flex-wrap items-center justify-end gap-x-3 gap-y-1">{DEMO_MODE && <span className="technical text-[10px] font-semibold tracking-wide text-violet"><span aria-hidden="true">●</span> DEMO DATA</span>}<BuilderIdControl />{DEMO_MODE && <button type="button" className="button-ghost !min-h-8 !border-transparent !px-2 text-[11px]" onClick={resetDemo} disabled={busy}><Icon name="reset" size={12} /> Reset demo</button>}</div>
    </div></header>
    <main className="page-wrap">
      ...
      {tab === "plan" && <>
        ...
        <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_322px]"><div className="min-w-0"><div className="mb-3 flex items-center justify-between"><div><div className="eyebrow">Optimized itinerary</div><h2 className="mt-1 text-[19px] font-semibold tracking-tight">Your week at a glance</h2></div>{plan && <button type="button" onClick={() => setTab("insights")} className="subtle-link">View insights →</button>}</div>
            ...
            <ProfilePanel profile={profile} onChange={setProfile} onDemo={() => { if (bootstrap) { setProfile(profileForRequest(bootstrap.profile)); setNotice("Platform Engineer profile loaded. Optimize your week to continue."); setError(""); } }} onOptimize={() => optimize()} busy={busy} demoAvailable={DEMO_MODE} demoReady={Boolean(bootstrap)} />
      ...
      {tab === "discover" && <DiscoverPanel profile={profile} />}
      ...
      {(pendingPlan || execution) && <div id="mutation-review" className="mt-6"><MutationPanel plan={pendingPlan} planId={pendingPlanId} result={execution} demoMode={DEMO_MODE} busy={busy} onConfirm={confirmMutation} onDismiss={dismissMutation} /></div>}
      <footer className="mt-12 border-t border-line pt-4 text-[10px] leading-5 text-muted"><div className="flex flex-wrap items-center justify-between gap-3"><p>Pathfinder · re:Invent 2026 · Offline-first planning</p><p>Live AWS attendee access pending registration · Live writes disabled</p></div></footer>
    </main>
  </div>;
}
